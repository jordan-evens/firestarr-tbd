...
# original from https://raw.githubusercontent.com/mapbox/rasterio/master/rasterio/merge.py
...
def merge(datasets, output_file, res=None, nodata=None, dtype=None, precision=10,
    ...
    # out_meta.update({"driver": "GTiff",
                     ...
                     # }
                    # )
    # with rasterio.open(output_file, "w", **out_meta) as to:
        # to.write(dest)

    # HACK: dt is set above when going through datasets
    dest_file = rasterio.open(output_file,
                              'w',
                              driver='GTiff',
                              width=output_width,
    ...
    if nodataval is not None:
        ...
        if np.dtype(dtype).kind in ('i', 'u'):
            ...
            if np.isnan(nodataval):
                ...
                inrange = (info.min <= nodataval <= info.max)
        # FIX: figure out how to do this to file
        # if inrange:
            # dest.fill(nodataval)
        # else:
            # warnings.warn(
    ...
    for idx, dataset in enumerate(datasets):
        ...
        dest_file.write(temp, window=dst_window)
    dest_file.close()


def to_mosaic(input, output_file, mask='*.tif', force=False):
    ...
    logging.debug('Output is {}'.format(output_file))
    if not force and os.path.exists(output_file):
        logging.debug("Mosaic {} already exists".format(output_file))
        return
    ...
    merge(src_files_to_mosaic, output_file, method='last')
