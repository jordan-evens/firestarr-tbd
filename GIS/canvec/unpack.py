...
def check_file(output, archive, member, use_crc=False):
    ...
    if not do_extract:
        ...
        if do_extract:
            ...
            logging.debug("Replacing incomplete file")
        elif use_crc:
            crc = "N/A"
            ...
            if do_extract:
                logging.debug("Replacing file with wrong CRC")
    if do_extract:
        logging.debug("[NEW]  " + member.filename)
        try:
        ...
        logging.debug("[OKAY] " + member.filename)
...
def check_archive(folder, mask, wrapper, file_mask, use_crc, output=None, force=False):
    ...
    if not force and os.path.exists(output):
        logging.debug("Output path {} already exists - not extracting".format(output))
        return
    logging.debug("Extracting to " + output)
