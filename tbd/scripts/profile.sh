...
rm -rf ${DIR_BUILD}
/usr/local/bin/cmake --no-warn-unused-cli -DCMAKE_EXPORT_COMPILE_COMMANDS:BOOL=TRUE -DCMAKE_BUILD_TYPE:STRING=RelWithDebInfo -S/appl/tbd -B${DIR_BUILD} -G "Unix Makefiles"
sudo /usr/local/bin/cmake --build ${DIR_BUILD} --config Debug --target all -j $(nproc) --
