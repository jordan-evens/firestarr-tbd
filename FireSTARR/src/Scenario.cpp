...
inline void doCondense(vector<InnerPos>& a)
{
  ...
  Idx x = a[0].x;
  Idx y = a[0].y;
  excList* tmp = newExcList();
  size_t i = 0;
  for (const auto p : a)
  {
//    logging::warning("Adding (%d, %d)/(%f, %f)", p.x, p.y, p.sub_x, p.sub_y);
//    logging::check_fatal(x != p.x || y != p.y, "Point is in wrong cell");
    insert({p.sub_x, p.sub_y}, tmp);
    ++i;
  }
//  logging::warning("Added %d points to exclusion list", i);
  edgeList* edges = peel(tmp);
  i = 0;
  edge* e = edges->first;
  a =  {};
  while (nullptr != e)
  {
    ++i;
    a.emplace_back(x, y, e->a.x, e->a.y);
    e = e->next;
  }
//  logging::warning("Have %d edges after peel", i);
//  i = 0;
//  e = edges->first;
//  edge* prev;
//  while (nullptr != e)
...
////    prev = e;
//    e = e->next;
////    // make sure previous second point in edge is first point in this edge
////    logging::check_fatal(nullptr != e && (prev->b.x != e->a.x && prev->b.y != e->a.y),
////                         "Previous end point (%f, %f) is not this start point (%f, %f)",
////                         prev->b.x, prev->b.y, e->a.x, e->a.y);
//  }
  delEdgeList(edges);
//  i = 0;
//  Node* n = tmp->head->fwd;
...
//    n = n->fwd;
//  }
//  logging::warning("Have %d points after peel", i);
//  a = {};
//  n = tmp->head->fwd;
//  while(nullptr != n)
//  {
////    logging::warning("Adding (%d, %d)/(%f, %f)", x, y, n->pt.x, n->pt.y);
//    a.emplace_back(x, y, n->pt.x, n->pt.y);
//    ++i;
//    n = n->fwd;
//  }
  delExcList(tmp);
